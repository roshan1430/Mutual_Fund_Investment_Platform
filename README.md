...
This repo now includes a root [`render.yaml`](./render.yaml) Blueprint so you can import the repository directly into Render.

What it creates:

- `mutual-fund-frontend`: public web service using the existing React build and Nginx SPA server
- `mutual-fund-backend`: public Spring Boot web service
- `mutual-fund-mysql`: private MySQL service with a persistent disk

Important note:

- The MySQL service uses a Render private service plus disk, which requires a paid plan.
- If you want a lower-cost setup, keep the frontend and backend on Render and point the backend at an external MySQL provider instead.
...
4. Render will detect `render.yaml` and propose the three services.
5. Provide mail env vars when prompted:
   - `SPRING_MAIL_HOST`
   - `SPRING_MAIL_USERNAME`
   - `SPRING_MAIL_PASSWORD`
   - `APP_MAIL_FROM`
6. Complete the Blueprint deploy.

### Render behavior in this setup

- The frontend proxies `/api` traffic to the backend over Render's internal network.
- The backend accepts the frontend Render URL through `APP_CORS_ALLOWED_ORIGINS`.
- The backend port automatically respects Render's `PORT` environment variable.
- MySQL credentials are generated by Render and shared with the backend through Blueprint env references.
